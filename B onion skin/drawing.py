...
"""GPU drawing for onion skin - Ultra Performance v2."""
...
def extract_all_meshes_merged(objects, depsgraph, wireframe=False):
    """Merge ALL objects into single vertex/index arrays. Returns numpy arrays."""
    if not HAS_NUMPY:
    ...
    for obj in objects:
        try:
            ...
            # Extract vertices using foreach_get (fastest method)
    ...
    # Merge arrays - keep as numpy for efficient GPU transfer
    merged_verts = np.vstack(all_verts)
    merged_indices = np.vstack(all_indices)
    
    # Convert to list format that batch_for_shader expects
    # Using tolist() is faster than list comprehension
    verts_list = merged_verts.tolist()
    indices_list = merged_indices.tolist()
...
def get_needed_frames(current, settings, scene):
    """Get list of needed frames, sorted by distance from current (closest first)."""
    ...
    # Sort by distance (closest frames first for priority caching)
...
def ensure_frames_cached(context):
    """Ensure all needed frames are cached, prioritizing closest frames."""
    scene = context.scene
    settings = scene.onion_skin_settings
    ...
    # Cache frames in priority order (closest first)
...
def calculate_alpha(base_alpha, t, falloff, curve):
    """Calculate alpha with falloff curve."""
    if curve == 'linear':
...
def draw_onion_skins():
    """Main draw callback - optimized for real-time performance."""
    context = bpy.context
    ...
    # Sort farthest first for proper transparency blending
    frames_to_draw.sort(key=lambda x: -x[3])
    
    shader = cache.get_shader()
    
    # Batch GPU state changes - set once before drawing all frames
    ...
    # Bind shader once for all frames
    shader.bind()
    
    # Pre-fetch colors and settings to avoid repeated attribute access
    ...
    # Draw all frames with single shader binding
